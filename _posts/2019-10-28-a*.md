---
title: "a*"
date: 2019-11-04T22:10:00-04:00
categories:
  - Algorithm
tags:
  - study
...
public:
	...
	vector<Node*> getNeighbor(Node node);
	double calDistance(Node n1, Node n2);
	void printDist(Node node);
...
vector<Node*> Graph::getNeighbor(Node node) {
	int x = node.x;
	int y = node.y;
	vector<Node*> result;
...
void Graph::printDist(Node node) {
	int cost = 0;
	Node* temp = &node;
